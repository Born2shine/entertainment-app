...
const Trending = ({TrendingMovies}) => {
  return (
    <section>
      <h3 className="py-4 font-normal trend">Trending</h3>
        ...
        {/* single trending movie */}
		{
			TrendingMovies?.map((movie, index) => (
				...
				<div className=" absolute bottom-3 left-3">
					<div className="flex items-center font-thin text-[10px] gap-3 text-gray-300">
					...
					<span className="relative">
						<span className={`w-[2px] h-[2px] rounded-full bg-gray-200 content-none absolute top-1/2 right-2.5 ${movie.rating !== 'E' && 'mr-2'}`}></span>
						{movie.rating}
					</span>
					</div>
