...
const Searched = ({SearchedMovies, Sortby}) => {
  return (
	<section>
      <h3 className="py-4 font-normal mt-4">Found {SearchedMovies?.length} Results for {Sortby} </h3>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-6">
        {/* single recommeded movie */}
        {SearchedMovies.length > 0 && SearchedMovies?.map((movie, index) => (
